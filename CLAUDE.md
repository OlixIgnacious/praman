...
3. `TRACKER.md`, `NOTES.md`, `plan.md` — **gitignored, local-only working files, not present in a fresh clone of this repo.** If they exist on disk, read them: `TRACKER.md` is the current "what's done vs. not" at a glance (check this first rather than assuming from file presence alone — a `sql/*.sql` file existing doesn't mean it's been run against Snowflake yet); `NOTES.md` lists pending manual Snowflake runs (non-interactive `cortex exec` auto-denies mutating SQL, so DDL/DML/grant scripts here are written but executed by a human interactively, not headless); `plan.md` is the flat file-level TODO mirroring `architecture.md`'s build plan, including a "Pending" section of open items beyond the day-by-day build (second-jurisdiction validation, RBAC role-boundary testing, pitch-only gaps). If they're absent, treat `architecture.md`'s Build plan section as the source of truth for status instead — everything shipped in the actual repo (SQL, `cortex_project/`, `eval/`, `demos/`) reflects real, deployed, tested state regardless of whether these tracking files are present.
...
**One detector, two consumers, literally.** `sql/detectors/01_zscore_udf.sql` defines a single `ZSCORE` UDF; `TRANSACTION_SIGNALS` (Stage 0, per-counterparty daily structuring/velocity) and `GL_OUTLIER_SIGNALS` (Stage 2, per-`ACCOUNT_CODE` monthly outliers vs. filing history) both call it. Never re-derive z-score anomaly logic inline in a skill — query these views. `CREDIT_EXPOSURE_SV`'s per-counterparty scale check reuses the same formula a third time, but not as a UDF call in a view: Snowflake rejects a Semantic View metric referencing another window-function metric, so that view only exposes the three baseline building blocks (`counterparty_account_baseline_mean`/`_stddev`/`_count`) and `SIGNAL_ASSURE_AGENT`'s `AI_SQL_GENERATION` instructions tell the agent the exact z-score formula to compute from them at query time — see `sql/semantic_views/README.md`.
...
`AUDIT_LOG` is append-only **by grant, not convention**: no role, anywhere, is ever granted `UPDATE`/`DELETE` on it. A maker-checker sign-off is recorded as a *new* `AUDIT_LOG` row (`OFFICER_SIGNOFF` role), never an `UPDATE` of the original run's row — preserve this if you touch sign-off logic. **Verified live, not just asserted** (`sql/rbac/verification_results.md`, 22/22 checks pass, 2026-09-13): true for every functional role (`ANALYST_READ`/`GOVERNANCE_WRITE`/`AUDIT_INSERT`/`OFFICER_SIGNOFF`), but `ACCOUNTADMIN` owns `AUDIT_LOG` and retains implicit override through Snowflake's ownership model — say "no operational role can modify the audit log," not "no one, including admins," in pitch material.
