...
**Overall:** 8 of 8 phases done. Days 15–17's eval went 7/12 → 11/12 after 4 of 5 fixes landed clean with zero regressions (`eval/results.md`); 1 gap remains (a real detector-calibration issue, two fixes identified, not attempted — decision point below). Only **Days 17–18 (rehearse & submit)** left.
...
- [x] Fixes redeployed and re-tested: **4/5 landed, 11/12 correct (up from 7/12), zero regressions.** Remaining `correct_but_anomalous` false positive is a real calibration gap (book-wide vs. per-counterparty scale baseline), not a wording issue — two fixes identified, not attempted (real detector work). Full detail: `eval/results.md`.
