...
- **Running the actual eval pass** — asking `SIGNAL_ASSURE_AGENT` each held-out question and scoring `MATCH_STATUS` into `EVAL_RESULTS` — is `run_eval.md` in this directory, including 3 additional cases built from the real `DIVERGENCE_DISCLOSURES` data (`sql/load_divergence_disclosures.sql`) calibrated to real-world materiality thresholds, not just the 9 injected ones.
- **Required before running `run_eval.md`:** `AUDIT_LOG.IS_EVAL` needs to actually be settable per-question. The original `SP_WRITE_AUDIT_LOG`/`SIGNAL_ASSURE_AGENT` combination always wrote `IS_EVAL=FALSE` — running eval traffic through the live agent as originally deployed would have contaminated the real audit trail, exactly what `architecture.md` says must not happen. Fixed: `SP_WRITE_AUDIT_LOG` gained an `IS_EVAL` parameter, and the agent's orchestration now reads a `[EVAL]` prompt marker to set it — see `sql/procedures/README.md` and `NOTES.md` for the redeploy steps, required before `run_eval.md` can run correctly.
