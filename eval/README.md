...
- **`sql/seed_injected_cases.sql` hasn't been run against Snowflake** — see `NOTES.md`.
- **The `EVAL_RESULTS` scoring harness itself** (`plan.md`'s next Days 15–17 item) — actually running Stage 0/2 against these nine cases and writing `MATCH_STATUS` rows. This catalogue is the input to that, not the harness.
- **The `structuring` case's `|z| >= 3` threshold is only verified structurally** (`tests/test_generator.py` checks the injected day's transaction count is ≥3x the counterparty's baseline daily average) — the actual SQL z-score can only be confirmed by running `SELECT * FROM TRANSACTION_SIGNALS WHERE COUNTERPARTY_ID = '<the injected one>'` live in Snowflake once the data is loaded.
