...
LANGUAGE SQL
EXECUTE AS OWNER
COMMENT = 'Accepts RETRIEVED_RULE_CHUNK_IDS as a comma-delimited VARCHAR (not ARRAY) because Cortex Agent generic tools with warehouse execution do not support array arguments. Parsed to ARRAY via SPLIT() on insert.'
AS
$$
BEGIN
  ...
  SELECT :RUN_ID, :APP_USER, :STAGE, :PROMPT_OR_QUESTION, :MODEL_VERSION,
         ...
         :QUERY_SNAPSHOT_ID, :OUTPUT, NULL, FALSE;
  RETURN 'OK';
END;
$$;
...
GRANT OWNERSHIP ON PROCEDURE SP_WRITE_AUDIT_LOG(VARCHAR,VARCHAR,VARCHAR,VARCHAR,VARCHAR,VARCHAR,VARCHAR,VARCHAR)
  TO ROLE AUDIT_INSERT COPY CURRENT GRANTS;

GRANT USAGE ON PROCEDURE SP_WRITE_AUDIT_LOG(VARCHAR,VARCHAR,VARCHAR,VARCHAR,VARCHAR,VARCHAR,VARCHAR,VARCHAR)
