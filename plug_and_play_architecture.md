...
The domain-coverage question (§0) and the "how do banks/analysts actually handle this today" question are being researched in parallel across multiple markets — see forthcoming `regulatory_landscape_research.md` for the consolidated findings (real report types, data domains, publication language, and current practitioner workflow per market).
