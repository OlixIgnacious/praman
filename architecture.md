...
15. Stand up the backend service (Python/TS) hosting the Coco Agent SDK, keypair auth against Snowflake, one endpoint per stage.
16. Build the thin review UI — one screen per stage: ask/answer, findings list, gap analysis, lineage + narrative.
17. Wire every Skill call to write an `AUDIT_LOG` row.
