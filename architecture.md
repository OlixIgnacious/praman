...
**Team:** solo build — see corrections this implies for scope/sequencing in the problem statement review.
...
- **A Snowflake "Skill" is a directory with a `SKILL.md` manifest** (YAML frontmatter — `name` + a dense, trigger-keyword-rich `description`, the actual activation mechanism — plus workflow/citation/escalation/audit sections). Lighter to author solo than the plan implies, and the "weaker as a moat" concern this note originally raised turned out to be about the flat, undeployed markdown files this project had at first, not an inherent property of skills: `cortex skill publish` registers a skill as a real Cortex Extension object in Snowflake's catalog, with its own `READ` grants — governed and shareable the same way a Semantic View is, once actually published (see `sql/procedures/README.md`-adjacent `NOTES.md` for this project's status). Even without publishing, this repo's four skills are already installable by anyone via `cortex skill add <this GitHub repo>` — confirmed locally with `cortex skill add ./skills/<name>` and `cortex skill list` showing all four as real, discovered `[EXTERNAL]` skills, not just design docs.
...
| `AUDIT_LOG` | `RUN_ID` (PK), `TIMESTAMP`, `USER`, `STAGE`, `PROMPT_OR_QUESTION`, `MODEL_VERSION`, `RETRIEVED_RULE_CHUNK_IDS` (array) or `QUERY_SNAPSHOT_ID`, `OUTPUT`, `HUMAN_DECISION`, `SIGNOFF_BY`, `SIGNOFF_AT`, `SIGNOFF_FOR_RUN_ID` (self-referencing FK — set only on a sign-off row, linking it back to the run it applies to) | Every Skill invocation, append-only | Compliance, internal audit, evidence pack export |
...
18. ~~Open design question: one Cortex Agent with several tools, or one agent per stage?~~ **Decided and built: one agent for Stage 0 + Stage 2** (they share Semantic Views, detector views, and `ANALYST_READ`). Full pros/cons documented separately during design.
