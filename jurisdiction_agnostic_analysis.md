...
Not done now — this is the checklist for when the second-jurisdiction work actually starts, ordered by what blocks what.

1. **Pick the jurisdiction and confirm the same five real-data requirements `data-sources.md` used for India exist for it** — a real circular, a published validation-rule/field-definition set, a taxonomy version delta, a filed-return-equivalent to anchor synthetic data to, and *some* form of corrected-filing ground truth (not necessarily RBI's specific divergence-disclosure mechanism — see the `DIVERGENCE_DISCLOSURES` note above). This is the same Day-1 gate `architecture.md`'s build plan used the first time; skipping it is what caused slippage risk before.
2. **Write a new circular parser** (new module, not a config change to `ingest/chunk_circular.py`) once a real document is in hand — study its actual structure first the way `ingest/README.md` did for RBI's, don't assume it nests the same way.
3. **Decide the `ACCOUNT_CODE` question up front**: reuse this project's exact convention (`ADVANCES_FUND`/`NPA_<classification>`/`NPA_PROVISION`) for the new jurisdiction's synthetic generator, or introduce a new one and update the three hardcoded consumers (`sql/semantic_views/03_credit_exposure_sv.sql`, `sql/detectors/03_gl_outlier_signals.sql`, `SIGNAL_ASSURE_AGENT.agent.yaml`) together, in one change. Reusing the convention is the cheaper path if the new jurisdiction's asset-classification categories map onto Substandard/Doubtful/Loss reasonably (Basel-derived, so plausible) — check this before assuming it.
4. **Parameterize currency**: drop the `DEFAULT 'INR'` in the three DDL files, and make the agent's response-formatting instruction read the currency from the data (or from a per-jurisdiction agent variant) instead of hardcoding ₹.
5. **New `RULE_CORPUS`/`LINE_ITEM_MAP`/anchor content** — mechanical once 1–4 are settled, using the same ingestion/seeding scripts and the same reconciliation algorithm in `generate_synthetic_data.py`, just against new source documents and figures.
6. **Decide whether `SIGNAL_ASSURE_AGENT` becomes multi-jurisdiction-aware in one agent, or a second agent gets deployed** — the same "one agent vs. many" question already decided once for Stage 0+2 (`.claude/plans/lets-decide-what-would-rippling-lighthouse.md`), now one level up: does one orchestration prompt reason correctly about two jurisdictions' worth of rules and account conventions, or does that risk the same kind of guardrail-blurring the Stage 0/2 combination already had to be designed around? Worth deciding deliberately rather than defaulting either way.
