...
6. **Decide whether `SIGNAL_ASSURE_AGENT` becomes multi-jurisdiction-aware in one agent, or a second agent gets deployed** — the same "one agent vs. many" question already decided once for Stage 0+2 during design, now one level up: does one orchestration prompt reason correctly about two jurisdictions' worth of rules and account conventions, or does that risk the same kind of guardrail-blurring the Stage 0/2 combination already had to be designed around? Worth deciding deliberately rather than defaulting either way.
