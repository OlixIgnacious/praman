...
- [~] **`SIGNAL_ASSURE_AGENT`** — 5 tools (`transaction_analytics`/`position_analytics`/`credit_exposure_analytics` over the three Semantic Views, `rule_corpus_search` over `RULE_CORPUS_SEARCH`, `write_audit_log` as a `generic`/procedure tool), two-block `instructions.orchestration` distilling `signal-query.SKILL.md` + `assure-return.SKILL.md` with a shared audit-logging tail. **Written, not yet deployed.** Spec: `cortex_project/SIGNAL_ASSURE_AGENT.agent.yaml`. Supersedes the `TRANSACTIONS_AGENT` spike (retire once verified — see `NOTES.md`).
- [~] `SP_WRITE_AUDIT_LOG` — the stored procedure `write_audit_log` calls; owned by `AUDIT_INSERT` (via ownership transfer, `EXECUTE AS OWNER`) so an `ANALYST_READ`-running agent can write an audit row without being granted `INSERT` on `AUDIT_LOG` directly. **Written, not yet run.** `sql/procedures/01_sp_write_audit_log.sql`; must run before the agent deploys.
- [ ] **Known open risk, not yet resolved:** `write_audit_log` is a tool the orchestration model chooses to call, not a guaranteed hook — no confirmed "always call this last" enforcement exists in the documented spec format. Mitigated with blunt, redundant prompting; the real signal is the verification step below (count `AUDIT_LOG` rows against test turns). If real gaps show up, the fallback is a reconciliation backstop — not built pre-emptively.
- [ ] Verify live in CoWork (checklist in `NOTES.md`), then retire `TRANSACTIONS_AGENT`
- [ ] Remaining custom backend, once the above lands: Stage 1/3's bespoke orchestration (circular ingestion, lineage-to-narrative) — everything else CoWork/Cortex Agent already covers declaratively
